...
import 'package:prior/pages/set_priority_page.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_auth.currentUser?.email ?? 'User Email not available'),
        actions: [
          //priority button
          IconButton(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => SetPriorityPage()),
              );
            },
    ...
    if (_auth.currentUser!.email != data['email']) {
      return Container(
        ...
        child: ListTile(
          ...
          onTap: () {
            //push the clicked user's UID to the chat page
            Navigator.push(
              context,
              MaterialPageRoute(
